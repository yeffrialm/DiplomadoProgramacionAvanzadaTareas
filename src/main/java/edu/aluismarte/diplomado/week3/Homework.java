...
public class Homework {
    ...
    public List<Quote> calculateQuotes(Loan loan) {
        ...
        for (int i = 1; i <= loan.getQuotes(); i++) {
            ...
            switch (loan.getLoanType()) {
                case FIXED, SAN ->
                        quote.setAmount(calculateAmount(loan.getLoanType(),
                                loan.getQuotes(), loan.getInterest(),
                                loan.getCapital()));
            }
            quotes.add(quote);
        }
    ...
    private BigDecimal calculateAmount(LoanType loanType, int quotes, BigDecimal interest, BigDecimal capitalAmount) {
        return switch (loanType) {
            case FIXED ->
                    capitalAmount.divide(new BigDecimal(quotes),
                            RoundingMode.CEILING).add(interest)
                            .setScale(2, RoundingMode.CEILING);
            case SAN ->
                    capitalAmount.add(interest).divide(new BigDecimal(quotes),
